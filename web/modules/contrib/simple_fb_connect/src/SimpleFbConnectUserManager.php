...
class SimpleFbConnectUserManager {
      ...
      if (array_key_exists($role, $disabled_roles)) {
        $this->loggerFactory
          ...
          ->warning('Facebook login for user @user prevented. Facebook login for role @role is disabled in module settings.', [
            '@user' => $drupal_user->getAccountName(),
            '@role' => $role,
          ]);
        return TRUE;
      }
   ...
   *   User's Facebook ID.
   *
   * @deprecated in 8.x-3.1 when the logic of this method was moved to method
   * createUser because the user creation failed when user_picture was required
   * field.
   *
   * @return bool
